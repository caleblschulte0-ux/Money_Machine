...
export default function BarklyKit({ toyId, playLabel, asleep, wants, disabled, onPress }: Props) {
  ...
  return <View style={styles.kit}>
    ...
    <View style={styles.dock} pointerEvents="none"><View style={styles.dockGloss} /><View style={styles.dockFront} /></View>
...
const styles = StyleSheet.create({
  kit: { position: 'absolute', left: 7, right: 7, bottom: -1, height: 76, flexDirection: 'row', justifyContent: 'space-around', alignItems: 'flex-end', paddingHorizontal: space.xs, zIndex: 8 },
  dockShadow: { position: 'absolute', left: 14, right: 14, bottom: -2, height: 24, borderRadius: radius.xl, backgroundColor: color.ink, opacity: 0.12 },
  dock: { position: 'absolute', left: 5, right: 5, bottom: 2, height: 46, borderRadius: radius.xl, backgroundColor: color.paper, borderWidth: 2, borderColor: color.line, overflow: 'hidden', ...elevation.low },
  dockGloss: { position: 'absolute', left: 17, right: 17, top: 5, height: 6, borderRadius: radius.pill, backgroundColor: color.glossSoft },
  dockFront: { position: 'absolute', left: 7, right: 7, bottom: 0, height: 8, borderTopLeftRadius: radius.md, borderTopRightRadius: radius.md, backgroundColor: color.line },
  slot: { minWidth: TAP_MIN + 26, minHeight: TAP_MIN + 20, alignItems: 'center', justifyContent: 'flex-end', paddingBottom: 2 },
  off: { opacity: 0.4 },
  wellShadow: { position: 'absolute', bottom: 5, width: 80, height: 30, borderRadius: radius.lg, backgroundColor: color.ink, opacity: 0.12 },
  wellShadowWanted: { opacity: 0.18 },
  well: { position: 'absolute', bottom: 9, width: 80, height: 31, borderRadius: radius.lg, backgroundColor: color.fill, borderWidth: 1.5, borderColor: color.line, overflow: 'hidden' },
  wellWanted: { backgroundColor: color.goldWell, borderColor: color.gold },
  wellGloss: { position: 'absolute', left: 9, right: 9, top: 4, height: 5, borderRadius: radius.pill, backgroundColor: color.gloss, opacity: 0.72 },
