...
from spec_e1 import BEATS, W, H, FPS, TOTAL

OUT = "out_e1"; RAWD = "../raw"; SR = 48000
...
def marks():
    ...
    for b, clip, tin, st, d, note in BEATS:
        ...
        # render_e1.py), so the sound is consistent with the figure-lock
        # sound rather than a new SFX vocabulary. Timed to the reticle's
        # own 0.55s converge window (render_e1.py: `t < 0.55`).
        if b == 'lock':
        ...
        # (hud.boot_pulse()/scan_sweep() in render_e1.py), distinct in
...
if __name__ == "__main__":
    ...
    marks()
    import score_e1; score_e1.main()
    import vo_e1; vo_e1.main()
    print("  sound")
    master("../out/ORI_Explainer1_TheIdea_master.mp4"); print("  mastered")
