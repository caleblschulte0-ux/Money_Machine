...
import hud as HUD
from spec_e1 import (BEATS, LABELS, ICE, TITLES, UI_OFF, WEARER_BEATS,
...
def main(only=None):
    ...
    # `hero` reads raw/IMG_HERO2.MOV -- the SAME source PNG as v32c's
    # IMG_HERO1.MOV (ai/hero/glasses_hero_chatgpt.jpg), built at this
    # video's own 6.0s duration into its OWN file, so v32c's frozen
    # IMG_HERO1.MOV (4.0s) is never touched. Built once already this
    # session; rebuilt here if a fresh checkout is missing it.
    hero_clip = os.path.join(RAW, "IMG_HERO2.MOV")
