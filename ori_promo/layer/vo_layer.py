#!/usr/bin/env python3
"""v37 "THE WORLD / THE LAYER" -- narration. Same synthesis/placement
machinery as one/vo_one.py, field/vo_field.py, walk/vo_walk.py,
map/vo_map.py (Piper, offline, en_US-lessac-high; placement is COMPUTED
from the actual synthesized duration each run). r184 requires a
COMPLETELY FRESH script whose sentence architecture and opening must not
reuse v35 or v36 -- spec_layer.py's VO_LINES is that fresh script.
"""
import os
import sys
import wave
...
from spec_layer import SECTIONS, VO_LINES, TOTAL
...
def main():
    ...
    for idx, (sec, offset, text) in enumerate(VO_LINES):
        ...
        a, sr = synth(text, f"out_layer/_vo_{idx:02d}_{sec}.wav")
        if sr != SR:
    ...
    with wave.open("out_layer/_vo.wav", "w") as w:
