#!/usr/bin/env python3
"""Narration for ORI Explainer #2 ("Fast Facts"), synthesized offline.
...
not a teaser. v32c stays exactly as it is. Where explain1/vo_e1.py speaks
full sentences over long calm holds, THIS script's LINES are clipped
fragments timed to short, fast-cut beats (2.0-4.0s each) -- the same
facts (software/hardware distinction, recognition, anchoring, the rental
model, the honest beta framing, extensibility), said in the fewest words
that still say them plainly, matched by an on-screen caption on almost
every beat (spec_e2.py's TITLES).
...
from spec_e2 import BEATS, TOTAL

SR = 48000
VOICE = "../vo/voices/en_US-lessac-high.onnx"

# (beat, seconds into that beat, line)
# EXPLAINER #2, fast fragment style. Same approved facts as explain1/v32c,
# said in as few words as possible -- no invented raise/terms/traction/
# partnership/deployment date, beta explicitly named as PROPOSED, not
# live.
LINES = [
    ("open", 0.15, "Falls Park. Sioux Falls, South Dakota."),
    ("intro", 0.15, "We're a software company. Not hardware."),
    # `hero` carries no VO of its own -- its label already reads
    # "THE HARDWARE" / "VISUALIZATION"; let it be looked at, not talked
    # over.
    ("on",   0.15, "Put them on."),
    # `lock` and `anchor` are the SAME UNBROKEN TAKE (spec_e2.py) -- one
    # continuous shot carrying two capability statements with no cut.
    ("lock", 0.15, "It recognizes what you're looking at."),
    ("anchor", 0.15, "Anchored to the real place around you."),
    ("rental", 0.20, "Destinations rent it. They don't buy it."),
    ("honest_stage", 0.20, "Right now, it's a proposal. Not live yet."),
    ("off",  0.15, "No phone in your face."),
    ("reach", 0.15, "It keeps working, wherever you go."),
    ("vision", 0.20, "The same platform. Any real place."),
]
...
def main():
    ...
    with wave.open("out_e2/_vo.wav", "w") as w:
