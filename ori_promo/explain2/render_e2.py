...
import hud as HUD
from spec_e2 import (BEATS, LABELS, ICE, TITLES, UI_OFF, WEARER_BEATS,
...
def main(only=None):
    ...
    # `hero` reads raw/IMG_HERO3.MOV -- the SAME source PNG as v32c's
    # IMG_HERO1.MOV and explain1's IMG_HERO2.MOV (ai/hero/glasses_hero_chatgpt.jpg),
    # built at THIS video's own 3.0s duration (this style's pace is quick
    # cuts, not long holds) into its OWN file, so neither IMG_HERO1.MOV
    # (4.0s, v32c, frozen) nor IMG_HERO2.MOV (6.0s, explain1) is touched.
    # Built once already this session; rebuilt here if a fresh checkout is
    # missing it.
    hero_clip = os.path.join(RAW, "IMG_HERO3.MOV")
