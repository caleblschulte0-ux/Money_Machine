#!/usr/bin/env python3
"""v34 "THE FIELD GUIDE" -- narration.

Same synthesis/placement machinery as one/vo_one.py (Piper, offline,
en_US-lessac-high; placement is COMPUTED from the actual synthesized
duration each run, never just the LINES offset -- see main()'s min_start
logic, copied verbatim because the reasoning is generic to any voice/any
script, not specific to v33's words). What's new is only the words
themselves: spec_field.py's VO_LINES, one line per beat, exactly matching
r168's field-guide narration script. No date, no measurement, no
attribution, no traction, no partnership, no deployment claim, no CTA --
same standing rule as v33.
"""
import os
import sys
import wave
...
from spec_field import BEATS, VO_LINES, TOTAL
...
def main():
    ...
    for idx, (beat, offset, text) in enumerate(VO_LINES):
        ...
        a, sr = synth(text, f"out_field/_vo_{idx:02d}_{beat}.wav")
        if sr != SR:
    ...
    with wave.open("out_field/_vo.wav", "w") as w:
