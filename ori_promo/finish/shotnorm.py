...
VEIL_HI    = (0.72, 0.99)   # highlight band for the hue restore
SHADOW_LO  = (0.03, 0.22)   # shadow band for the same guard -- see apply()
...
def _lo_weight(l, lo=SHADOW_LO[0], hi=SHADOW_LO[1]):
    """Smoothstep ramp over the SHADOW band, falling from 1 at black."""
    t = np.clip((l - lo) / max(hi - lo, 1e-6), 0.0, 1.0)
...
def apply(bgr, p):
    ...
    # The same guard is needed at BOTH ends, and the shadow half was missing.
    # Subtracting a near-neutral veil from a DARK coloured pixel raises its
    # relative chroma, and the luminance lift then amplifies that: pixels
    # measuring BGR 39/32/22 at value 0.15 in the source came out as vivid
    # saturated blue. It showed as blue blobs on a backpack, an arch and a
    # treeline -- and it is what I first misread as blue cars being real
    # content in Film D's plate. Some of that blue IS real; this much of it
    ...
    lsrc  = _lum(src)
    k = np.clip(_hi_weight(lsrc) + _lo_weight(lsrc), 0.0, 1.0)[..., None]
