...
def dakota_a(dur=5.4):
    ...
    els = [El(woman, 1240, 745, 372, shadow_alpha=52)]
    occ = plate_occluder("raw/IMG_6804.MOV", 23.5,
                        [(560, 1080), (600, 730), (900, 700),
                         (1300, 758), (1920, 838), (1920, 1080)])
    # demo pace: the activation is the signature — let it expand slowly
    render("trailer/seq/v7_dak_a", els, dur, anchor=(1240, 720), t0=0.9,
           front_dur=1.35, front_max=880, occluder=occ)


def dakota_b(dur=3.4):
    ...
    els = [El(tipi, 1420, 790, 470, shadow=0.62, shadow_alpha=44)]
    render("trailer/seq/v7_dak_b", els, dur, anchor=(1420, 780), t0=0.35,
...
def settle_a(dur=4.0):
    """Queen Bee Mill area stones foreground — ONLY the wagon resolves,
    partly behind the real stone pile."""
    wagon = load_mat(f"{A}/wagon.png", lift=0.86)
    els = [El(wagon, 1291, 918, 560, shadow=0.82, shadow_alpha=74)]
    occ = plate_occluder("raw/IMG_6805.MOV", 27.5,
                        [(1020, 1080), (1030, 792), (1180, 706),
                         (1420, 686), (1720, 712), (1860, 792),
                         (1870, 1080)],
                        crop=SETA_CROP)
    render("trailer/seq/v7_set_a", els, dur, anchor=(1300, 880), t0=0.5,
...
def settle_b(dur=3.4):
    """Tighter: ONE settler beside part of the wagon."""
    woman = load_mat(f"{A}/settler_woman.png", lift=1.12)
    wagon = load_mat(f"{A}/wagon.png", lift=0.78, washv=0.10)
    els = [El(wagon, 1382, 900, 540, shadow=0.7, shadow_alpha=34),
           El(woman, 880, 892, 470, shadow_alpha=66)]
    occ = plate_occluder("raw/IMG_6805.MOV", 31.9,
                        [(973, 1080), (983, 800), (1140, 700),
                         (1440, 680), (1780, 720), (1830, 1080)],
                        crop=SETB_CROP)
    render("trailer/seq/v7_set_b", els, dur, anchor=(880, 890), t0=0.35,
...
def ice(dur=6.6):
    ...
    mam = load_mat(f"{A}/mammoth_clean.png", washv=0.30, blur=0.55)
    el = El(mam, 640, 470, 245, shadow=0.7, shadow_alpha=46)
    ...
    mist.alpha_composite(Image.fromarray(strip).crop((0, 0, 1920, 90)), (0, 432))
    render("trailer/seq/v7_ice", [el], dur, anchor=(640, 476), t0=1.5,
           quiet=True, fade_resolve=0.6, mist=mist)
