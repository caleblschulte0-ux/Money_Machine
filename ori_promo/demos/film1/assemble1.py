#!/usr/bin/env python3
"""DEMO 1 — end card, concat, sound, master."""
import os, sys, subprocess, json, re
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import numpy as np, cv2
from PIL import Image, ImageDraw, ImageFont
from spec1 import BEATS, W, H, FPS, TOTAL
...
def marks():
    """One soft tick as each recognition LOCKS. The sound is the confirmation."""
    from spec1 import ANCHORS
    n = int(TOTAL*SR)+SR
    a = np.zeros(n, np.float32)
    def put(at, sig):
        i = int(at*SR); a[i:i+len(sig)] += sig[:max(0, n-i)]
    for b, clip, tin, st, d, note in BEATS:
        for (pt, ti, sub, t0, off) in ANCHORS.get(b, []):
            put(st+t0-0.55, _tick(int(0.20*SR), 520, 380, 0.055, 17.0))   # scan starts
            put(st+t0,      _tick(int(0.34*SR), 250, 150, 0.20, 8.0))     # lock
    a = a[:int(TOTAL*SR)]
    import wave
...
def master(dst):
    with open("concat1.txt","w") as fh:
        for b, clip, tin, st, d, note in BEATS:
            fh.write(f"file '{os.path.abspath(OUT)}/{b}_t.mp4'\n")
    subprocess.run(["ffmpeg","-v","error","-y","-f","concat","-safe","0","-i","concat1.txt",
...
if __name__ == "__main__":
    ...
    master("../out/ORI_DEMO_1_Through_The_Glass_master.mp4"); print("  mastered")
