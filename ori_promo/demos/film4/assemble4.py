#!/usr/bin/env python3
"""DEMO 4 — end card, concat, sound, master.

The sound is deliberately NOT Demo 1's. Demo 1 ticks once per recognition,
because recognition is a discrete event. This film measures, so it gets a
travelling tone under the range sweep, a soft settle when a material class
closes, and a low body when the ground opens. Same instrument family, a
different job.
"""
...
from spec4 import BEATS, LABELS, APERTURE, W, H, FPS, TOTAL
from render4 import SCHED

OUT = "out4"; RAWD = "../raw"; SR = 48000
...
def bed():
    ...
    for b, clip, tin, st, d, note in BEATS:
        src, s_in = (clip, tin) if clip else ("6798", 6.5)
...
def marks():
    ...
    for b, clip, tin, st, d, note in BEATS:
        ...
        if not s: continue
        if s.get("shells"):
            put(st + s["shells"][0], _tone(int(0.30*SR), 340, 250, 0.055, 11.0))
        if s.get("sweep"):
            s0, s1 = s["sweep"]
            put(st + s0, _glide(int((s1-s0)*SR), 96, 232, 0.085))
        if s.get("classes"):
            for j in range(3):
                put(st + s["classes"][0] + 0.9 + j*0.45,
                    _tone(int(0.22*SR), 620, 430, 0.055, 15.0))
        if s.get("ap"):
            put(st + s["ap"][0], _tone(int(0.80*SR), 88, 52, 0.19, 3.6))
        if b in LABELS:
            put(st + LABELS[b][3], _tone(int(0.30*SR), 250, 150, 0.15, 8.0))
    a = a[:int(TOTAL*SR)]
...
def master(dst):
    ...
    subprocess.run(["ffmpeg","-v","error","-y","-f","concat","-safe","0","-i","concat4.txt",
...
if __name__ == "__main__":
    ...
    master("../out/ORI_DEMO_4_Deep_Time_master.mp4"); print("  mastered")
