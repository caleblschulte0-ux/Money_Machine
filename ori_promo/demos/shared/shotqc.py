...
def motion(clip, tin, dur, raw="raw"):
    ...
    d = []
    cum = np.zeros(2)
    # cv2.phaseCorrelate MULTIPLIES BOTH INPUTS BY THE WINDOW IN PLACE. Every
    # frame here is used twice -- as b in one pair and as a in the next -- so
    # without these copies frame i arrives at its second use already tapered,
    # and gets tapered again. Verified: a 64x64 float32 array comes back with
    # a max delta of 254 levels after one call. The effect is a correlation
    # peak biased toward the centre, i.e. UNDER-reported motion, on a gate
    # whose whole job is to catch motion. Found 2026-08-26 while building
    # Demo 4; every number this module printed before that date was measured
    # with the bug and has been re-measured since.
