#!/usr/bin/env python3
"""ORI EXPLAINER #1 -- "THE IDEA". Render.

Adapted directly from one/render_one.py (v32c's engine) -- same engine
modules (shotnorm, filmlook, labelkit, arlabel, depthtools), same
compose()/frames_of()/encode() machinery, unchanged. This file exists
instead of editing one/render_one.py so v32c's frozen, ChatGPT-approved
render code is never touched by a completely separate video's work.
FIGURES/ICE/GEN_ICE/WEARER_BEATS are all empty here (same as v32c), so
none of the era-compositing or ice-grade code paths in compose() ever
fire -- they are dead weight carried over from the shared engine, not
something this video uses.
"""
import os
import subprocess
import sys
...
import hud as HUD
from spec_e4 import (BEATS, LABELS, ICE, TITLES, UI_OFF, WEARER_BEATS,
...
#
# THIS VIDEO'S ADJACENCIES, reasoned the same way v32c's were, verified
# by direct frame extraction after the first render (not assumed):
# every beat boundary that crosses to a DIFFERENT source clip (or jumps
# to a different, distant in-point on the same clip) dips to black --
# `intro` (open->intro), `hero` (intro->hero), `on` (hero->on),
# `rental` (anchor->rental), `honest_stage` (rental->honest_stage),
# `off` (honest_stage->off), `reach` (off->reach), `vision`
# (reach->vision, even though it revisits IMG_6804, at a different
# in-point 17s away from `rental`'s -- not the same continuous moment).
# `lock` and `anchor` are the ONE exception, same reasoning as v32c: they
# are literally the same continuous take as `on` continuing forward on
# its own clock (8.4 -> 10.9 -> 15.4), so dissolving from the previous
# beat's last held frame is dissolving the shot into itself -- harmless
# by construction, not something to special-case away.
DIP_TO_BLACK = {"intro", "hero", "on", "rental", "honest_stage", "off", "reach", "vision"}
...
def compose(beat, dur, frames, prev_last=None, global_i=0):
    ...
    for i, f in enumerate(frames):
        ...
        if lab and lpath:
            ...
            if beat in RECOGNITION_LABELS and t < 0.55:
                # THE CONVERGING RETICLE arlabel.py's own docstring names as
                # the recognition step ("the convergence IS the recognition
                # happening") -- built early in this project, never actually
                # called from here. `lock` is the one real, unmodified
                # object this film recognises (the falls); giving it the
                # convergence it was designed for, right before the label
                # settles on the same tracked anchor.
                AR.reticle(d, (cx, cy), t, col=CYAN)
            if t >= t0:
        ...
        # something (hero/on/lock/anchor); every pure-B-roll beat stays
        # exactly as restrained as before -- a park does not get a
        # hologram.
        if beat == "hero":
...
def main(only=None):
    ...
    # `hero` reads raw/IMG_HERO5.MOV -- the SAME source PNG as v32c's
    # IMG_HERO1.MOV, explain1's IMG_HERO2.MOV, explain2's IMG_HERO3.MOV
    # and explain3's IMG_HERO4.MOV (ai/hero/glasses_hero_chatgpt.jpg),
    # built at THIS video's own 4.0s pace into its OWN file, so none of
    # the other four is touched. Built once already this session;
    # rebuilt here if a fresh checkout is missing it.
    hero_clip = os.path.join(RAW, "IMG_HERO5.MOV")
