...
from spec_e4 import BEATS, W, H, FPS, TOTAL

OUT = "out_e4"; RAWD = "../raw"; SR = 48000
...
def marks():
    ...
    for b, clip, tin, st, d, note in BEATS:
        ...
        # render_e4.py), so the sound is consistent with the figure-lock
        # sound rather than a new SFX vocabulary. Timed to the reticle's
        # own 0.55s converge window (render_e4.py: `t < 0.55`).
        if b == 'lock':
        ...
        # (hud.boot_pulse()/scan_sweep() in render_e4.py), distinct in
...
if __name__ == "__main__":
    ...
    marks()
    import score_e4; score_e4.main()
    import vo_e4; vo_e4.main()
    print("  sound")
    master("../out/ORI_Explainer4_TheQA_master.mp4"); print("  mastered")
