import { DialogueEngine } from '../src/barkley/dialogue';
import { BarkleyMemory } from '../src/barkley/memory';
...
async function makeEngine(provider: DialogueProvider) {
  const memory = new BarkleyMemory(createInMemoryStore(), 'default');
  await memory.load();
...
describe('DialogueEngine', () => {
  it('runs a full round: reply parsed, turns recorded, memories merged', async () => {
    ...
    expect(state.turns.map((t) => t.role)).toEqual(['user', 'barkley']);
    expect(state.userFacts.join(' ')).toContain('Caleb');
  });

  it('feeds prior facts back into the next prompt (Barkley remembers)', async () => {
    const seen: string[] = [];
    const spy: DialogueProvider = {
      ...
      async complete(req) {
        ...
        return JSON.stringify({
          speech: 'Noted.',
          remember: { user_facts: ["Your person's name is Caleb."], barkley_memories: [] },
        });
      },
    };
