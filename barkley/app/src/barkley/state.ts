/**
 * Barkley's state machine and internal drives.
 *
 * One reducer owns every transition — no animation booleans scattered through
 * the UI. The UI dispatches BarkleyEvents; this module decides what Barkley is
...
export const DEFAULT_STATS: BarkleyStats = {
  mood: 65,
  energy: 80,
  hunger: 40,
  affection: 50,
...
export function freshSnapshot(now: number): BarkleySnapshot {
  return { state: 'idle', stats: { ...DEFAULT_STATS }, updatedAt: now };
}

const clamp = (v: number) => Math.max(0, Math.min(100, Math.round(v)));

function adjust(stats: BarkleyStats, delta: Partial<BarkleyStats>): BarkleyStats {
  return {
 ...
 * Rates are per hour and intentionally gentle — Barkley away for a weekend is
 * hungry and a bit mopey, not dead. Decay is capped at 48h so a long absence
 * doesn't zero everything out.
 */
export function decayStats(stats: BarkleyStats, elapsedMs: number): BarkleyStats {
...
/**
 * Baseline: what Barkley drifts back to when nothing is happening.
 * Needs override mood — a very hungry or exhausted dog doesn't idle happily.
 */
export function baselineState(stats: BarkleyStats): BarkleyState {
...
const TRANSIENT: BarkleyState[] = ['happy', 'excited', 'annoyed', 'eating', 'playing'];

export function isTransient(state: BarkleyState): boolean {
  return TRANSIENT.includes(state);
}

export function reduce(snap: BarkleySnapshot, event: BarkleyEvent): BarkleySnapshot {
  ...
  switch (event.type) {
    ...
    case 'TICK': {
      ...
      const next: BarkleySnapshot = { ...snap, stats: decayed, updatedAt: event.now };
...
export function settleDelayMs(state: BarkleyState): number {
  switch (state) {
...
/**
 * Ambient body actions for a state — what Barkley's body does on its own.
 * The dialogue model can add more per reply; these are the defaults.
 */
export function ambientActions(state: BarkleyState): BodyAction[] {
  switch (state) {
...
export function describeStats(stats: BarkleyStats): string {
  const parts: string[] = [];
  parts.push(
