 ...
 *   EXPO_PUBLIC_BARKLEY_BACKEND_URL production proxy base URL (preferred)
 *   EXPO_PUBLIC_BARKLEY_MODEL       override dialogue model id
 *
...
export function createProviders(): ProviderSet {
  ...
  const baseURL = process.env.EXPO_PUBLIC_BARKLEY_BACKEND_URL;
  const model = process.env.EXPO_PUBLIC_BARKLEY_MODEL;
