/**
 * The home screen — Barkley's room. Barkley is the product: he owns the
...
import { useBarkley } from '../hooks/useBarkley';
import BarkleyView from './BarkleyView';
import SettingsSheet from './SettingsSheet';
import { BarkleyState } from '../barkley/types';

const STATE_LABEL: Record<BarkleyState, string> = {
  idle: '',
  listening: 'listening…',
  thinking: 'thinking…',
  speaking: '',
...
export default function BarkleyRoom() {
  const barkley = useBarkley();
  const [settingsOpen, setSettingsOpen] = useState(false);
  const [typed, setTyped] = useState('');

  const { snapshot, actions, lastExchange, partialTranscript, error, busy, sttAvailable } = barkley;
    ...
    >
      ...
      <View style={styles.captions}>
        {listening && partialTranscript ? (
          ...
          <>
            ...
            <Text style={styles.barkleyLine} numberOfLines={4}>{lastExchange.barkleyText}</Text>
          </>
        ) : (
          <Text style={styles.hint}>
            {sttAvailable ? 'Hold TALK and say hi to Barkley.' : 'Type something and say hi to Barkley.'}
          </Text>
        )}
        {error && <Text style={styles.error}>{error}</Text>}
      </View>

      {/* Barkley owns the middle of the screen */}
      <View style={styles.stage}>
        <BarkleyView state={snapshot.state} actions={actions} />
      ...
      {sttAvailable ? (
        <Pressable
          ...
          disabled={busy}
          onPressIn={barkley.startTalk}
          onPressOut={barkley.stopTalk}
        >
        ...
        <View style={styles.typeRow}>
          <TextInput
            ...
            placeholder="Say something to Barkley…"
      ...
      <View style={styles.actionsRow}>
        <ActionButton label="PLAY" onPress={barkley.play} disabled={busy} />
        <ActionButton label="FEED" onPress={barkley.feed} disabled={busy} />
        <ActionButton
          label={snapshot.state === 'sleepy' ? 'WAKE' : 'SLEEP'}
          onPress={barkley.sleepToggle}
          disabled={busy}
        />
      </View>
      ...
      <SettingsSheet
        ...
        memory={barkley.memorySnapshot()}
        dialogueProviderName={barkley.dialogueProviderName}
        sttAvailable={sttAvailable}
        onForgetEverything={barkley.forgetEverything}
      />
    </KeyboardAvoidingView>
  );
}
...
const styles = StyleSheet.create({
  ...
  barkleyLine: { fontSize: 17, fontWeight: '600', color: '#2E2A26', textAlign: 'center', lineHeight: 23 },
