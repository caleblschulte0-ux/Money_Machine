...
translates those into animation; a physical Barkley translates the exact same
...
Mobile UI                 app/src/ui/*             screens, controls
  ↓
Barkley Interaction Layer app/src/hooks/useBarkley.ts   glue: owns engine + state
  ↓
Speech-to-Text            app/src/providers/stt/*  SpeechToTextProvider
  ↓
Barkley AI / Dialogue     app/src/barkley/dialogue.ts + providers/dialogue/*
  ↓
Memory                    app/src/barkley/memory.ts + storage/*
  ↓
Text-to-Speech            app/src/providers/tts/*  TextToSpeechProvider
  ↓
Animation / Emotion       app/src/barkley/state.ts + src/animation/* + ui/BarkleyView.tsx
```
...
## The Barkley state machine

`app/src/barkley/state.ts`. One reducer-style controller, no scattered booleans.
...
interactions (feed/play/sleep/talk), and feed the prompt so Barkley's behavior
varies naturally. Deliberately not a full Tamagotchi simulation.

## Memory (three tiers)

`app/src/barkley/memory.ts`, persisted through the `KeyValueStore` abstraction
...
3. **Barkley memories** — experiences Barkley believes he shared with the user
...
`ui/BarkleyView.tsx` is a **placeholder renderer**: Barkley drawn from plain RN
`View`s + the `Animated` API, following the locked design's blocky geometry.
It renders from exactly two inputs — `BarkleyState` + active `BodyAction`s —
via the `BarkleyRenderer` contract in `app/src/animation/renderer.ts`. Replacing
...
- **Rive (recommended)** — state-machine-native, tiny runtime, inputs map 1:1
  onto `BarkleyState`/`BodyAction`, strong Expo support. Best MVP-to-production
...
- Storage is namespaced per profile (`barkley/profile/<id>/…`) so parental
